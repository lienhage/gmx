...
contract Vault is ReentrancyGuard, IVault {
    ...
    struct Position {
        uint256 size;
        uint256 collateral;
        uint256 averagePrice; // position merge
        uint256 entryFundingRate;
        uint256 reserveAmount; // how many token borrowed
        int256 realisedPnl;
    ...
    /// @notice global short related variables
    // where is global long?
    ...
    function setFundingRate(uint256 _fundingInterval, uint256 _fundingRateFactor, uint256 _stableFundingRateFactor) external override {
        ...
        // how long time funding fee is charged
    ...
    // set USDG amount to target amount, in what circumctances this will be called
    ...
    // what user can do with USDG rather than GLP?
    function buyUSDG(address _token, address _receiver) external override nonReentrant returns (uint256) {
        // if in manager mode, the msg.sender must be the manager
        _validateManager();
        _validate(whitelistedTokens[_token], 16);
        // why set and reset this in a single transaction?
        useSwapPricing = true;

        // get how many token transferred to this contract
        uint256 tokenAmount = _transferIn(_token);
        _validate(tokenAmount > 0, 17);

        // update funding rate of this token
        updateCumulativeFundingRate(_token, _token);

        // minimum value of tokens sent
        uint256 price = getMinPrice(_token);

        uint256 usdgAmount = tokenAmount.mul(price).div(PRICE_PRECISION);
        // 算了两遍USDG amount，分别是没收手续费的和收了手续费的
    ...
    function swap(address _tokenIn, address _tokenOut, address _receiver) external override nonReentrant returns (uint256) {
        ...
        /// @notice make sure have enough reseves for leverage
    ...
    /// @param _indexToken target token
    /// @param _sizeDelta position size delta：in USD？
    function increasePosition(address _account, address _collateralToken, address _indexToken, uint256 _sizeDelta, bool _isLong) external override nonReentrant {
        ...
        // if the four params are the same, the key is the same. In in one direction of a collateral
        // token and index token, there can only have one position
        bytes32 key = getPositionKey(_account, _collateralToken, _indexToken, _isLong);
        Position storage position = positions[key];

        // long use max price, short use min price
        ...
        /// @notice get average price and merge positions
        ...
        // make sure position value is enough to pay for the fee
        ...
        // reserveAmount = userCollateral + LPLend
        position.reserveAmount = position.reserveAmount.add(reserveDelta);
        _increaseReservedAmount(_collateralToken, reserveDelta);

        // 只有做多会增加poolAmount和guaranteedUSD，做空只会增加globalShortSize
        if (_isLong) {
            // guaranteedUsd stores the sum of (position.size - position.collateral) for all positions
            // if a fee is charged on the collateral then guaranteedUsd should be increased by that fee amount
            // since (position.size - position.collateral) would have increased by `fee`
            /// @notice 仓位中用户实际向LP借了多少钱，以USD记账，手续费也是LP借给用户的部分
            _increaseGuaranteedUsd(_collateralToken, _sizeDelta.add(fee));  // + 仓位增加数量 - 抵押品增加数量
            _decreaseGuaranteedUsd(_collateralToken, collateralDeltaUsd);
            // treat the deposited collateral as part of the pool
            /// @notice why? I thought poolAmount is LP's funds
            _increasePoolAmount(_collateralToken, collateralDelta); // + 抵押品增加数量 ，也就就是本次开多仓净流入数量
            // fees need to be deducted from the pool since fees are deducted from position.collateral
            // and collateral is treated as part of the pool
            // fee 是LP借给用户的，所以从抵押品数量中扣除
    ...
    function _decreasePosition(address _account, address _collateralToken, address _indexToken, uint256 _collateralDelta, uint256 _sizeDelta, bool _isLong, address _receiver) private returns (uint256) {
        ...
        if (position.size != _sizeDelta) {
            ...
            // 空头为什么没有guaranteedUSD一说？
            if (_isLong) {
                // 减仓前后抵押品的变化量（减少）
                // deltaGuaranteedUSD = (nextSize - nextCollateral) - (size - collateral)
                //                    = -sizeDelta + oldCollateral - nextCollateral
        ...
        // 清仓
            ...
            if (_isLong) {
                // nextSize - 0 - （size - collateral) = -sizeDelta + collateral
    ...
    function liquidatePosition(address _account, address _collateralToken, address _indexToken, bool _isLong, address _feeReceiver) external override nonReentrant {
        ...
        if (_isLong) {
            ...
            // 空单，之前就已经把用户的抵押品和LP借的钱算到poolAmount里了，所以这里只扣fee
    ...
    // return a cumulated funding rate during a period of time since last updated to now
    function getNextFundingRate(address _token) public override view returns (uint256) {
        // if has not pass a funding interval yet
        if (lastFundingTimes[_token].add(fundingInterval) > block.timestamp) { return 0; }

        // Calculate how many itervals we have
        ...
        // factor * reservedAmount * intervals / poolAmount
        // todo: reservedAmount/poolAmount
    ...
    // for longs: nextAveragePrice = (nextPrice * nextSize) / (nextSize + delta)
    // for shorts: nextAveragePrice = (nextPrice * nextSize) / (nextSize - delta)
    // 平均价格： nextPrice x nextSize / (oldSize / oldPrice x nextSize + sizeDelta)
    function getNextGlobalShortAveragePrice(address _indexToken, uint256 _nextPrice, uint256 _sizeDelta) public view returns (uint256) {
        ...
        // 如果价格下跌，做空有盈
        bool hasProfit = _nextPrice < averagePrice;

        // 全局空头头寸的USD数量
        uint256 nextSize = size.add(_sizeDelta);

        // 价格下跌，delta为正，价格上涨，delta为负
        uint256 divisor = hasProfit ? nextSize.sub(delta) : nextSize.add(delta);

        // 价格下跌：nextAveragePrice = nextPrice x nextSize / (nextSize - delta)
        // 价格上涨：nextAveragePrice = nextPrice x nextSize / (next + delta)
        return _nextPrice.mul(nextSize).div(divisor);
    }
    ...
    // delta: part of the divisor to calculate average price, 或者可以理解为原仓位价值的变化量的绝对值
    function getDelta(address _indexToken, uint256 _size, uint256 _averagePrice, bool _isLong, uint256 _lastIncreasedTime) public override view returns (bool, uint256) {
        ...
        /// @notice price has been fetched from previous steps
        uint256 price = _isLong ? getMinPrice(_indexToken) : getMaxPrice(_indexToken);
        /// get price diff
        uint256 priceDelta = _averagePrice > price ? _averagePrice.sub(price) : price.sub(_averagePrice);
        // positionValueDelta = size * priceDelta
        /// @notice sizeDelta = positionValueDelta / averagePrice?
        ...
        // delta / size <= minBps / BASIS_POINTS_DIVISOR
    ...
    // 1. 计算用户关仓手续费
    // 2. 计算用户P&l
    // 3. 如果用户盈利且仓位为空单，从poolAmount中支付P&L；如果用户亏损且为空单，从用户的抵押品中扣除用户亏损给poolAmount
    // 4. 计算要给用户转多少钱：用户要提取的抵押品数量 + 用户的结算盈亏
    function _reduceCollateral(address _account, address _collateralToken, address _indexToken, uint256 _collateralDelta, uint256 _sizeDelta, bool _isLong) private returns (uint256, uint256) {
        ...
        // scope variables to avoid stack too deep errors
        {
            ...
            // delta = priceDelta / averagePrice x size
            // sizeDelta x delta / size = sizeDelta x priceDelta / averagePrice
            // 本次减少的仓位的价值变动（盈亏情况）
        ...
        if (hasProfit && adjustedDelta > 0) {
            ...
            if (!_isLong) {
                ...
                // 稳定币做空，将P&L转成对应的稳定币数量，减少poolAmount
    ...
    function _validateTokens(address _collateralToken, address _indexToken, bool _isLong) private view {
        /// @notice can only use non-stable tokens for long position like WBTC
        if (_isLong) {
        ...
        /// can only use stable token for short position like USDC
    ...
    function _increasePoolAmount(address _token, uint256 _amount) private {
        // fee reserves is excluded from pool amounts
    ...
    function _increaseReservedAmount(address _token, uint256 _amount) private {
        ...
        // make sure reserve amount doesn't exceeded poolAmounts
